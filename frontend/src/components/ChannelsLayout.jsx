  ...
  return (
    <Col xs={5} sm={4} md={3} xxl={2} className="h-100 border-end pt-5 px-0 bg-light">
      <div className="d-flex justify-content-between align-items-center mb-2 ps-4 pe-2">
        ...
        <Button onClick={() => dispatch(showModal({ type: 'adding' }))} className="p-2 text-primary btn-group-vertical" variant={null}>
      ...
      <Nav className="px-2" as="ul">
        {channels
          .map((channel) => (
            <RenderChannel props={{ channel, currentChannelId }} key={channel.id} />
          ))}
      </Nav>
    </Col>
  );
